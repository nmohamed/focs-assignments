...
a. Is this algorithm any of: **greedy**, ~~recursive, dynamic programming (top down), dynamic programming (bottom up)?~~ --> It's greedy since it doesn't try to find an optimal solution

b. If the text contains a word that is longer than `max_width`, what does this algorithm do? --> it prints it anyway on its own line.

c. Does this algorithm produce an optimal result? (Do you have enough information to answer this question?) --> There isn't enough information for this. You could have printed the first output as:
```
aaa
bb cc
ddddd
```
instead, but there is no definition of what's "optimal" here. The question only says to break lines. You could also add a hyphen for words that are too long to maintain wrap length (and do it at the end of a syllable) but the question is not specific enough.
...
a. Is this algorithm any of: ~~greedy, recursive~~, dynamic programming (top down), ~~dynamic programming (bottom up)~~? --> It's top down dynamic programming since it tries to find the most optimal solution
